...
from app.temperature.crud import create_temp_record, get_all_temp, get_temp_for_city
...
async def read_all_temperatures(db: AsyncSession = Depends(get_session)):
    return await get_all_temp(db=db)


@router.get("/{city_id}/", response_model=list[schemas.TemperatureRead])
async def read_temp_for_city(city_id: int, db: AsyncSession = Depends(get_session)):
